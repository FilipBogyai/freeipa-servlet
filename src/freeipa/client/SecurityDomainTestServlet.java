...
/**
 * A servlet to test that the security domain required by the authenticator can
 * successfully authenticate.
 * 
 * @author darran.lofthouse@jboss.com
 * @version $Revision$
 */
@WebServlet(name = "DomainServlet", urlPatterns = { "/domain/" }, loadOnStartup = 1)
public class SecurityDomainTestServlet extends HttpServlet {
    ...
    private void testDomain(final String securityDomain, final PrintWriter writer) {
        ...
        try {
            ...
            writer.println("privatny");

            byte[] token = null;
            
            for (KerberosTicket ticket : subject.getPrivateCredentials(KerberosTicket.class)) {
                writer.println("principals:");
                writer.print(ticket.getServer());
                writer.print(ticket.getClient());
                token = Subject.doAs(subject, new SPNEGOTokenGeneratorAction(ticket.getClient()));
            ...
            String response = ipaClient.certificateRequest(new URL(
"https://vm-144.idm.lab.eng.brq.redhat.com/ipa/json"));
